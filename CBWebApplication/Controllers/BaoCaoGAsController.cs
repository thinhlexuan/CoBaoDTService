...
namespace CBWebApplication.Controllers
{
    ...
    [ApiController]
    public class BaoCaoGAsController : ControllerBase
    {
        private readonly CoBaoDTContext _context;

        public BaoCaoGAsController(CoBaoDTContext context)
        {
            _context = context;
        }
        ...
        [HttpGet]
        [Route("GetBCVanDung")]
        public async Task<ActionResult<IEnumerable<ViewBcvanDung>>> GetBCVanDung(string maDV, DateTime ngayBD, DateTime ngayKT, string loaiMayID, string tuyenID)
        {
        ...
        public async Task<ActionResult<IEnumerable<ViewBcGioDon>>> GetBCGioDon(string MaDV, int loaiBC, DateTime ngayBD, DateTime ngayKT)
        {
            if (loaiBC < 5)
            {
                ...
                var query1 = from dt in _context.DoanThongGA
                             join cb in _context.CoBaoGA on dt.DoanThongID equals cb.CoBaoID
        ...
        public async Task<ActionResult<IEnumerable<ViewBcBKDauMo>>> GetBCBKDauMo(string MaDV, short LoaiDM, int tuThang, int denThang, int nam)
        {
            ...
            else
            {
                var query = from dt in _context.DoanThongGA
                            join cb in _context.CoBaoGA on dt.DoanThongID equals cb.CoBaoID
                            join dm in _context.CoBaoGADM on dt.DoanThongID equals dm.CoBaoID
